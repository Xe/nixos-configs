...

{
  ...
  services.github-runner = {
    ...
    name = "xe-tstest-integration-vms-hostname-${config.networking.hostName}";

    # Replace this with the path to the GitHub Actions runner token on
    # your disk.
    tokenFile = "/run/keys/ts-oss-ghaction-token";
  };

  within.secrets.ghrunner = {
    source = ./secret/ghtoken + "-${config.networking.hostName}";
    dest = "/run/keys/ts-oss-ghaction-token";
    owner = "ghrunner";
    group = "keys";
    permissions = "0440";
  };
  ...
  systemd.services.github-runner = {
    after = [ "ghrunner-key.service" ];
    wants = [ "ghrunner-key.service" ];
    serviceConfig = {
